...
Plugins are applications that run inside of the CMS with UI and Backend components.
...
## Project Structure
The project contains multiple directories and files, but the most important ones for plugin development are `ui` and `backend`.

- The backend directory contains the backend code for the plugin.
  Here, you will write your custom REST endpoints and server-side logic.
  To install backend dependencies, navigate to the root of the project and run `npm install --save PACKAGE_NAME`.
- The `ui` directory contains the frontend code for the plugin.
...
Both BCMS and BCMS Plugins use [Purple Cheetah](https://github.com/becomesco/purple-cheetah) for the backend. This means that you will use Purple Cheetah to create controllers and middleware.
...
1. Create a controller/middleware object in the backend directory.
  For example, let's create a hello-world.ts file with the following code:
...
You can find the frontend code for the plugin in the `ui` directory.
The BCMS UI is built with Vue 3 and uses `.tsx` syntax.
...
Developers can access the [SDK](https://github.com/becomesco/cms-sdk) via window.bcms.sdk.*. This makes it easy to access the backend API.
...
- Run `npm run bundle` to bundle the backend and frontend code and create a plugin package.
