...
<CodeGroup>```bash npm install @becomes/cms-cli -g ```</CodeGroup>

To verify that the installation went well, run:

<CodeGroup>```bash bcms --help ```</CodeGroup>
...
<CodeGroup>```bash bcms --cms create ```</CodeGroup>
...
<Note>
  ...
  that Docket is running on your system. Run ```docker ps -a``` to list all
...
Once all containers are started, you can open your browser and navigate to <code className='text-green dark:text-yellow'>localhost:8080</code>.
