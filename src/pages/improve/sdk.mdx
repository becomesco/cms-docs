...
Caching becomes increasingly more important when there are more clients connected to the same Backend simultaneously.
For example, consider two clients, **C-one** and **C-two**, who requested and received a copy of all widgets.
If C-one updates a widget, C-two's cache must also be updated to remain in sync with the Backend.
The BCMS SDK uses sockets to solve this problem.
With every update on the Backend, all active clients get informed via socket.
